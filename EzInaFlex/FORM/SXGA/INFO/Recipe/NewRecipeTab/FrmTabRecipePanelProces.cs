...
namespace EzIna
{
    public partial class FrmTabRecipePanelProcess : Form
    {    
        DataGridView m_DGV_PARAM;
        GUI.UserControls.ExpandDataGridView m_DGV_GROUP;
        public FrmTabRecipePanelProcess()
        {
            InitializeComponent();
        ...
        private void Form_VisibleChanged(object sender, EventArgs e)
        {
            if (Visible)
            {
                FA.MGR.RecipeMgr.TreeView_Init(treeView_Menu,
                                               FA.DEF.eRecipeCategory.PROCESS.ToString(),
                                               MF.RCP_Modify.GetSubCatagoryStringList(FA.DEF.eRecipeCategory.PROCESS));
            }
        }
        ...
        {
          //FA.RCP_Modify.PROCESS_SCANNER_HALF_PERIOD.Value=5;
        }

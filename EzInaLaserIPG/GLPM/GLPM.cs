...
{
    public sealed partial class GLPM : CommunicationBase, LaserInterface
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public GLPM(string a_strName, Commucation.SerialCom.stSerialSetting a_Setting, int a_iPacketDoneTimeout) : base(a_Setting, a_iPacketDoneTimeout)
        {
        ...
        {
            AddPacket(GLPM_CMD.STATUS, "", false, enumPacketType.GetValue, true);
            AddPacket(GLPM_CMD.READ_INT_TRIGGER_FREQ, "", false, enumPacketType.GetValue, true);
            AddPacket(GLPM_CMD.READ_EXT_TRIGGER_FREQ, "", false, enumPacketType.GetValue, true);
            AddPacket(GLPM_CMD.CASE_TEMP, "", false, enumPacketType.GetValue, true);
            AddPacket(GLPM_CMD.HEAD_TEMP, "", false, enumPacketType.GetValue, true);
            AddPacket(GLPM_CMD.READ_CURRENT_SET_POINT, "", false, enumPacketType.GetValue, true);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        {
            AddPacket(GLPM_CMD.READ_FIRMWARE_VERSION, "", false, enumPacketType.GetValue, false);
            AddPacket(GLPM_CMD.READ_EMIISION_CONTROL, "", false, enumPacketType.GetValue, false);
            AddPacket(GLPM_CMD.READ_POWER_SOURCE, "", false, enumPacketType.GetValue, false);
            AddPacket(GLPM_CMD.READ_MIN_TRIGGER_FREQ, "", false, enumPacketType.GetValue, false);
            AddPacket(GLPM_CMD.READ_MAX_TRIGGER_FREQ, "", false, enumPacketType.GetValue, false);
        ...
        private void AddPacket<T>(GLPM_CMD a_CMD, T a_Value, bool a_bExistSetValue, enumPacketType a_Type, bool a_bLoop)
        {
        ...
        protected override bool ParsingPacketData(byte[] RecvData)
        {
            ...
            try
            {
                ...
                string[] strValues;
                GLPM_CMD CurrentCmd;
                CurrentCmd = (GLPM_CMD)base.CurrentPacket.iCMD;
        ...
        private void ParsingValue(GLPM_CMD a_CMD, enumPacketType a_Type, string a_SetValue, string[] a_RecvValue)
        {
            ...
            switch (a_CMD)
            {
                case GLPM_CMD.STATUS:
                    {
                        int.TryParse(a_RecvValue[0], out iCovertTemp);
                        m_Status.Data = (uint)iCovertTemp;
                    }
                    break;
                case GLPM_CMD.EMISSION_ON:
                    break;
                case GLPM_CMD.EMISSION_OFF:
                    break;
                case GLPM_CMD.CASE_TEMP:
                    {
                        float.TryParse(a_RecvValue[0], out m_fCaseTemp);
                    }
                    break;
                case GLPM_CMD.HEAD_TEMP:
                    {
                        float.TryParse(a_RecvValue[0], out m_fHeadTemp);
                    }
                    break;
                case GLPM_CMD.BACK_REFLECTION:
                    {
                        float.TryParse(a_RecvValue[0], out m_fBackReflection);
                    }
                    break;
                case GLPM_CMD.SET_CURRENT_SET_POINT:
                    {
                        float.TryParse(a_RecvValue[0], out m_fSetPoint);
                    }
                    break;
                case GLPM_CMD.READ_CURRENT_SET_POINT:
                    {
                        float.TryParse(a_RecvValue[0], out m_fSetPoint);
                    }
                    break;
                case GLPM_CMD.SET_TRIGGER_MODE:
                    break;
                case GLPM_CMD.READ_TRIGGER_MODE:
                    break;
                case GLPM_CMD.SET_TRIGGER_FREQ:
                    break;
                case GLPM_CMD.READ_INT_TRIGGER_FREQ:
                    {
                        double.TryParse(a_RecvValue[0], out m_fINT_Trigger_FREQ);
                    }
                    break;
                case GLPM_CMD.READ_EXT_TRIGGER_FREQ:
                    {
                        double.TryParse(a_RecvValue[0], out m_fEXT_Trigger_FREQ);
                    }
                    break;
                case GLPM_CMD.READ_MIN_TRIGGER_FREQ:
                    {
                        int.TryParse(a_RecvValue[0], out m_iMinTriggerFREQ);
                    }
                    break;
                case GLPM_CMD.READ_MAX_TRIGGER_FREQ:
                    {
                        int.TryParse(a_RecvValue[0], out m_iMaxTriggerFREQ);
                    }
                    break;
                case GLPM_CMD.SET_SOURCE_MODULATION:
                    break;
                case GLPM_CMD.READ_SOURCE_MODULATION:
                    break;
                case GLPM_CMD.SET_POWER_SOURCE:
                case GLPM_CMD.READ_POWER_SOURCE:
                    {
                        int.TryParse(a_RecvValue[0], out iCovertTemp);
                        m_bPowerSourceCtrl = iCovertTemp > 0;
                    }
                    break;
                case GLPM_CMD.SET_EMIISION_CONTROL:
                case GLPM_CMD.READ_EMIISION_CONTROL:
                    {
                        int.TryParse(a_RecvValue[0], out iCovertTemp);
                        m_bEmissionCtrl = iCovertTemp > 0;
                    }
                    break;
                case GLPM_CMD.RESET_ERR:
                    break;
                case GLPM_CMD.READ_FIRMWARE_VERSION:
                    {
        ...
        public bool EmissionOnOff
        {
            set
            {
                if (m_bEmissionCtrl == true)
                {
                    if (value == true)
                    {
                        AddPacket(GLPM_CMD.EMISSION_ON, "", false, enumPacketType.SetValue, false);
                    }
                    else
                    {
                        AddPacket(GLPM_CMD.EMISSION_OFF, "", false, enumPacketType.SetValue, false);
                    }
                }
            }
        }
        ...
        public TRIG_MODE TriggerMode
        {
            ...
            set
            {
                ...
                {
                    case TRIG_MODE.INT:
                        AddPacket(GLPM_CMD.SET_TRIGGER_MODE, 1, true, enumPacketType.SetValue, false);
                        break;
                    case TRIG_MODE.EXT:
                        AddPacket(GLPM_CMD.SET_TRIGGER_MODE, 0, true, enumPacketType.SetValue, false);
                        break;
                    default:
                        break;
                }
        ...
        public double RepetitionRate
        {
            ...
            set
            {
                if (value >= m_iMinTriggerFREQ && m_iMaxTriggerFREQ >= value)
                    AddPacket(GLPM_CMD.SET_TRIGGER_FREQ, (int)value, true, enumPacketType.SetValue, false);
            }
        }
        ...
        public double SetDiodeCurrent
        {
            ...
            set
            {
                ...
                {
                    AddPacket(GLPM_CMD.SET_CURRENT_SET_POINT, value, true, enumPacketType.SetValue, false);
                }
            }
        }
        /// <summary>
        ...
        public float BackReflection
        {
            get { return m_fBackReflection; }
        }
        ...
        public bool Modulation
        {
            ...
            set
            {
                AddPacket(GLPM_CMD.SET_SOURCE_MODULATION, value == true ? 1 : 0, true, enumPacketType.SetValue, false);
            }
        }
        /// <summary>
        /// false : EXT , true =INT
        ...
        public bool EmissionControl
        {
            ...
            set
            {
                AddPacket(GLPM_CMD.SET_EMIISION_CONTROL, value == true ? 1 : 0, true, enumPacketType.SetValue, false);
            }
        }
        /// <summary>
        /// false : EXT , true =INT
        ...
        public bool PowerSourceControl
        {
            ...
            set
            {
                AddPacket(GLPM_CMD.SET_POWER_SOURCE, value == true ? 1 : 0, true, enumPacketType.SetValue, false);
            }
        }
        public void Reset()
        {
            AddPacket(GLPM_CMD.RESET_ERR, "", false, enumPacketType.SetValue, false);
        }

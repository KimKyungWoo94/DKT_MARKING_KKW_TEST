...
namespace EzIna
{
    public partial class FrmVisionLIBOfPanel : Form
    {
        #region [ Delegate ]
        ...
        public FrmVisionLIBOfPanel(string a_strVisionName)
        {
            InitializeComponent();
            this.TopLevel = false;
        ...
        private void OnMouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (!AXIS.RX.Status().IsMotionDone || !AXIS.Y.Status().IsMotionDone)
                return;
						EzInaVision.VisionLibBaseClass pLib=FA.MGR.VisionMgr.GetLib(m_strVisionName);
						if(pLib==null)
								return;
                ...
                case "imageBoxEx_VisionOfPanel":
                    if (imageBoxEx_VisionOfPanel.IsPointInImage(e.Location) && chkbox_ClickMove.Checked)
                    {
                        ...
                        ptTargetPos = imageBoxEx_VisionOfPanel.PointToImage(e.X, e.Y);
												dMX = pLib.GetRelXPosToMoving(ptTargetPos);
												dMY = pLib.GetRelYPosToMoving(ptTargetPos);

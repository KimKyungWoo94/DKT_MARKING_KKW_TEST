...
using System.Collections.Concurrent; //kai
	...
	public class ImageBox : VirtualScrollableControl
	{
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			if (this.AllowPainting)
			{
                ...
                #region kai [ Display - Cross line , ROI ]
                // draw the selection

                //{{

               
			    this.DrawCrossLine(e.Graphics);
		...
		public virtual Rectangle GetInsideViewPort(bool includePadding)
		{
			...
			//kai
			//{{
			// ClientSize : 컨트롤 클라이언트 영여의 높이와 너비를 가져오가나 설정합니다.
			//}}
        ...
        ///   Draws a cross line.
        /// </summary>
        /// <param name="g">The graphics to draw the cross line to.</param>
        protected virtual void DrawCrossLine(Graphics g)
		{
            ...
            viewport = this.GetImageViewPort();
			scaled = this.GetScaledRectangle(0, 0, Image.Width, Image.Height);
            offsetX = viewport.Left + this.Padding.Left + this.AutoScrollPosition.X;
            offsetY = viewport.Top + this.Padding.Top + this.AutoScrollPosition.Y;

            offsetEdgeX = viewport.Left + this.Padding.Left;
            offsetEdgeY = viewport.Top + this.Padding.Top;


            //             //전체 크로스라인
            // 			Point ptBeginHoriz = new Point(scaled.Left + offsetX, scaled.Top + offsetY + (scaled.Height / 2));
            // 			Point ptEndhHoriz = new Point(scaled.Left + offsetX + scaled.Width, scaled.Top + offsetY + (scaled.Height / 2));
            // 
            // 			Point ptBeginVert = new Point(scaled.Left + offsetX + (scaled.Width / 2), scaled.Top + offsetY);
            // 			Point ptEndVert = new Point(scaled.Left + offsetX + (scaled.Width / 2), scaled.Top + offsetY + scaled.Height);
            // 
            ...
            // 			})
            // 			{
            // 				g.DrawLine(pen, ptBeginHoriz, ptEndhHoriz);
            // 				g.DrawLine(pen, ptBeginVert, ptEndVert);
            // 			}
            //작은 크로스라인
            Point ptCenter = new Point(scaled.Width / 2, scaled.Height / 2);
            Point ptSize = new Point( (ptCenter.X / 10) / 2, (ptCenter.Y / 10) / 2);
            if (ptSize.X > ptSize.Y)
                ptSize.Y = ptSize.X;
            else
                ptSize.X = ptSize.Y;

            Point ptBeginHoriz = new Point(ptCenter.X - ptSize.X + offsetX, scaled.Top + offsetY + (scaled.Height / 2));
            Point ptEndhHoriz = new Point(ptCenter.X + ptSize.X + offsetX, scaled.Top + offsetY + (scaled.Height / 2));

            Point ptBeginVert = new Point(scaled.Left + offsetX + (scaled.Width / 2), ptCenter.Y - ptSize.Y + offsetY);
            Point ptEndVert = new Point(scaled.Left + offsetX + (scaled.Width / 2), ptCenter.Y + ptSize.Y + offsetY);

  
            Point ptLT = new Point(offsetEdgeX, offsetEdgeY);
            Point ptRT = new Point(offsetEdgeX + viewport.Width, offsetEdgeY);

            Point ptLB = new Point(offsetEdgeX, offsetEdgeY + viewport.Height);
            Point ptRB = new Point(offsetEdgeX + viewport.Width, offsetEdgeY + viewport.Height);
            ...
            })
            {
                ...
                g.DrawLine(pen, ptLT.X, ptLT.Y, ptLT.X + ptSize.X, ptLT.Y);
                g.DrawLine(pen, ptLT.X, ptLT.Y, ptLT.X, ptLT.Y + ptSize.Y);

                g.DrawLine(pen, ptLB.X, ptLB.Y, ptLB.X + ptSize.X, ptLB.Y);
                g.DrawLine(pen, ptLB.X, ptLB.Y, ptLB.X, ptLB.Y - ptSize.Y);

                g.DrawLine(pen, ptRT.X, ptRT.Y, ptRT.X - ptSize.X, ptRT.Y);
                g.DrawLine(pen, ptRT.X, ptRT.Y, ptRT.X, ptRT.Y + ptSize.Y);

                g.DrawLine(pen, ptRB.X, ptRB.Y, ptRB.X - ptSize.X, ptRB.Y);
                g.DrawLine(pen, ptRB.X, ptRB.Y, ptRB.X, ptRB.Y - ptSize.Y);
            }
        }


        #region kai [ Move roi to center of image ]
        /// <summary>
        ///   ROI move to center of image.
        /// </summary>
        /// <param name="g">The graphics to draw the cross line to.</param>
        protected virtual void RoiMoveToCenterOfImage()
		{
            RectangleF currRect = SelectionRegion;
            if (currRect == Rectangle.Empty)
                return;


            int nImageCenterX = Image.Width / 2;
			int nImageCenterY = Image.Height / 2;

			currRect.X = nImageCenterX - currRect.Width / 2;
			currRect.Y = nImageCenterY - currRect.Height / 2;

            SelectionRegion = currRect;

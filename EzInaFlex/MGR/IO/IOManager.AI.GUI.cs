...
namespace EzIna.IO
{
    ...
    {
        public void InitializeAIDataGridView(DataGridView a_DatagirdView)
        {
            InitializeDataGridViewDefalutSet(a_DatagirdView);
            a_DatagirdView.DataSource = new BindingList<EzIna.IO.AI>(this.AIList);
        ...
        /// 
        public void UpdateDataGrid_AIColumn(DataGridView a_DatagirdView)
        {
            if (a_DatagirdView != null)
            {
                foreach (DataGridViewRow Item in a_DatagirdView.Rows)
                {
                    Item.Cells["Addr0"].Value = this.AIList[Item.Index].AddressInfo.m_strParamList[0];
                    Item.Cells["Addr1"].Value = this.AIList[Item.Index].AddressInfo.m_strParamList[1];
                    Item.Cells["Addr2"].Value = this.AIList[Item.Index].AddressInfo.m_strParamList[2];
                    Item.Cells["Addr3"].Value = this.AIList[Item.Index].AddressInfo.m_strParamList[3];
                    Item.Cells["Addr4"].Value = this.AIList[Item.Index].AddressInfo.m_strParamList[4];
                }
            }
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="a_DatagirdView"></param>
        public void UpdateDataGrid_AIValue(DataGridView a_DatagirdView)
        {
            if (a_DatagirdView != null)
            {
                if (a_DatagirdView.Rows.Count == this.DIList.Count)
                {
                    foreach (DataGridViewRow Item in a_DatagirdView.Rows)
                    {
                        Item.Cells["Value"].Value = string.Format("{0:F3}",this.AIList[Item.Index].Value);
                    }
                }
            }
        }      
